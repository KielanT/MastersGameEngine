...
namespace Engine
{
    ...
    Mesh::Mesh(DirectX11Renderer* renderer, const std::string& fileName, bool requireTangents /*= false*/)
    {
        ...
        for (unsigned int m = 0; m < scene->mNumMeshes; ++m)
        {
            ...
            glm::vec3* assimpPosition = reinterpret_cast<glm::vec3*>(assimpMesh->mVertices);
            ...
            {
                *(glm::vec3*)position = *assimpPosition;
                position += subMesh.vertexSize;
                ++assimpPosition;
            }

            glm::vec3* assimpNormal = reinterpret_cast<glm::vec3*>(assimpMesh->mNormals);
            ...
            if (requireTangents)
            {
                glm::vec3* assimpTangent = reinterpret_cast<glm::vec3*>(assimpMesh->mTangents);
                ...
                {
                    *(glm::vec3*)tangent = *assimpTangent;
            ...
            if (assimpMesh->GetNumUVChannels() > 0 && assimpMesh->HasTextureCoords(0))
            {
                ...
                while (uv != uvEnd)
                {
                    *(glm::vec2*)uv = glm::vec2(assimpUV->x, assimpUV->y);
            ...
            if (mHasBones)
            {
                if (assimpMesh->HasBones())
                {
                    ...
                    {
                        node.offsetMatrix = glm::mat4(1.0f); 
                    }
                    ...
                    for (unsigned int i = 0; i < assimpMesh->mNumBones; ++i)
                    {
                        ...
                        for (nodeIndex = 0; nodeIndex < mNodes.size(); ++nodeIndex)
                        {
                            if (mNodes[nodeIndex].name == boneName)
                            {
                                ...
                                ConvertMatrices(mNodes[nodeIndex].offsetMatrix, assimpBone->mOffsetMatrix);
                                glm::transpose(mNodes[nodeIndex].offsetMatrix);
    ...
    unsigned int Mesh::ReadNodes(aiNode* assimpNode, unsigned int nodeIndex, unsigned int parentIndex)
    {
        ...
        ConvertMatrices(node.defaultMatrix, assimpNode->mTransformation);
		
        glm::transpose(node.defaultMatrix); //.Transpose(); // Assimp stores matrices differently to this app
